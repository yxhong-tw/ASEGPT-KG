...
def generate(input_text: Union[str, List[str]], model_name_or_path: str,
    ...
    if isinstance(input_text, str):
        outputs = llm.generate(get_prompt_func(input_text, args.rationale),
                               sampling_params)
    else:
        input_prompts = [
            get_prompt_func(p, args.rationale) for p in input_text
        ]
...
def main(args: argparse.Namespace):
    ...
    sample_articles = sample_articles[sample_articles.columns.intersection(
        ['article_content', 'label', 'labeler'])]

    predictions = generate(
        input_text=sample_articles['article_content'].tolist(),
        model_name_or_path=args.model,
        get_prompt_func=partial(get_prompt, strategy=args.strategy),
        args=args)
    try:
